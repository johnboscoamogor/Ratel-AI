...
const modeIcons: Record<GenerationMode, React.ReactNode> = {
  ...
  [GenerationMode.IMAGE_ANIMATION]: <FilmIcon className="w-5 h-5" />,
...
const VeoPromptForm: React.FC<PromptFormProps> = ({
  onGenerate,
  initialValues,
}) => {
  ...
  const promptPlaceholder = {
    ...
    [GenerationMode.IMAGE_ANIMATION]: 'Describe the animation for the image...',
  ...
  const selectableModes = [
    ...
    GenerationMode.IMAGE_ANIMATION,
    GenerationMode.FRAMES_TO_VIDEO,
    GenerationMode.REFERENCES_TO_VIDEO,
  ];

  const renderMediaUploads = () => {
    if (generationMode === GenerationMode.IMAGE_ANIMATION) {
       return (
  ...
  switch (generationMode) {
    case GenerationMode.TEXT_TO_VIDEO:
      ...
      break;
    case GenerationMode.IMAGE_ANIMATION:
      isSubmitDisabled = !startFrame || !prompt.trim();
       if (isSubmitDisabled) {
        tooltipText = 'An image and a prompt are required.';
      }
      break;
